...
import { queueBorrowedEmail, queueReturnedEmail, queueExtendedEmail } from '@/lib/email/notifications';
...
export async function issueBook(formData: FormData) {
    ...
    const { data: newBorrowing, error } = await supabase.from('borrowings').insert({
        book_id: bookId,
        member_id: memberId,
...
export async function returnBook(borrowingId: string) {
    ...
    const { data: borrowing } = await supabase
        .from('borrowings')
        .select('book_id, books(name, author, library_id, libraries(name)), members(name, email)')
        .eq('id', borrowingId)
        .single();
...
export async function extendBorrowing(borrowingId: string) {
    ...
    const { data: borrowing } = await supabase
        .from('borrowings')
        .select('due_date, extended_at, books(name, author, libraries(name)), members(name, email)')
        .eq('id', borrowingId)
        .single();
