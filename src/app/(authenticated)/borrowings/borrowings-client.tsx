...
import { Plus, RotateCcw, BookMarked, Search } from 'lucide-react';
import { format } from 'date-fns';

interface BorrowingsClientProps {
    initialBorrowings: Borrowing[];
    books: Book[];
    members: Member[];
}

export function BorrowingsClient({ initialBorrowings, books, members }: BorrowingsClientProps) {
        ...
        <div className="space-y-6">
                    ...
                    <DialogContent className="sm:max-w-[500px]">
                        <form onSubmit={handleIssueBook}>
                            <DialogHeader>
                                ...
                                <DialogDescription>
                                    Find member by email or phone, select a book, and update phone number.
                                </DialogDescription>
                            </DialogHeader>
                            <div className="grid gap-4 py-4">
                                ...
                                <div className="grid gap-2">
                                        ...
                                        <div className="border rounded-md max-h-32 overflow-y-auto">
                                            {filteredMembers.map(m => (
                                                ...
                                                >
                                                    ...
                                                    <div className="text-xs text-muted-foreground">{m.email} • {m.phone || 'No phone'}</div>
                                                </div>
                                            ))}
                                        </div>
                                    )}
                                    {selectedMember && (
                                        <div className="p-2 bg-muted rounded-md text-sm">
                                            Selected: <strong>{selectedMember.name}</strong> ({selectedMember.email})
                                        </div>
                                    )}
                                </div>
                                ...
                                {/* Phone Update */}
                                <div className="grid gap-2">
                                    <Label htmlFor="phone">Phone Number (Required for each borrow) *</Label>
                                    <Input
                                        id="phone"
                                        type="tel"
                                        value={phone}
                                    ...
                                    >
                                        ...
                                        {availableBooks.map(book => (
                                            <option key={book.id} value={book.id}>
                                                {book.name} by {book.author} ({book.available_copies} available)
                                            </option>
                                        ))}
                                    </select>
                                </div>
            ...
            {borrowings.length === 0 ? (
                <div className="text-center py-12">
                    ...
                    <p className="text-muted-foreground">Issue a book to a member to get started</p>
                </div>
            ) : (
                <Table>
                    <TableHeader>
                    ...
                    <TableBody>
                        {borrowings.map((borrowing) => (
                            <TableRow key={borrowing.id}>
                                <TableCell className="font-medium">{borrowing.books?.name || 'Unknown'}</TableCell>
                                ...
                                <TableCell>
                                    <span className={isOverdue(borrowing.due_date) ? 'text-destructive font-medium' : ''}>
