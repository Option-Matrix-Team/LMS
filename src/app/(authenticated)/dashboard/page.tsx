...
import { BookOpen, Users, BookMarked, AlertCircle, Building2, UserCheck } from 'lucide-react';
...
export default async function DashboardPage() {
    ...
    let libraryCount = 0;
    let totalUsers = 0;
    let libraryStats: { id: string; name: string; books: number; members: number; users: number; borrowings: number }[] = [];
    ...
    if (isSystemOperator) {
        ...
        const [booksRes, membersRes, borrowingsRes, overdueRes] = await Promise.all([
            adminClient.from('books').select('id', { count: 'exact', head: true }).eq('library_id', libraryId),
            adminClient.from('members').select('id', { count: 'exact', head: true }).eq('library_id', libraryId),
            adminClient.from('borrowings')
                .select('id, books!inner(library_id)', { count: 'exact', head: true })
                        ...
                        <CardContent>
                            {libraryStats.length === 0 ? (
                                <div className="text-center py-8">
                                    ...
                                    <p className="text-muted-foreground">No libraries created yet</p>
                                    <Link href="/libraries">
                                ...
                                <Table>
                                    <TableHeader>
                                        <TableRow>
                                            ...
                                            <TableHead className="text-center">Active Borrowings</TableHead>
                                        </TableRow>
                                    </TableHeader>
                                    <TableBody>
                                        {libraryStats.map((lib) => (
                                            <TableRow key={lib.id}>
                                                ...
                                                <TableCell className="text-center">{lib.users}</TableCell>
                                                <TableCell className="text-center">{lib.books}</TableCell>
                                                <TableCell className="text-center">{lib.members}</TableCell>
                                                <TableCell className="text-center">{lib.borrowings}</TableCell>
                                            </TableRow>
                                        ))}
                                    </TableBody>
                                </Table>
